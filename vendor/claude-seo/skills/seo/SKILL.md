...
| `/seo write-page <topic> <keywords>` | → Delegates to `/blog write` in page mode |
| `/seo write-blog <keyword>` | → Delegates to `/blog write` with SEO context |
| `/seo content-plan <url_or_niche>` | → Delegates to `/blog calendar` + `/blog cluster` |
| `/seo keyword-research <seed>` | Conduct semantic keyword research (volume, difficulty, intent, clusters) |
...
For content-writing commands (`write-blog`, `write-page`, `content-plan`):
1. Load audit context from `Projects/<domain>/final/health-score.json` if available
2. Load keyword research from `Projects/<domain>/final/keyword-opportunity-map.md` if available  
3. Delegate to the corresponding `/blog` command with injected context
4. Blog engine handles template selection, research, writing, quality scoring, and delivery

For other individual commands, load the relevant sub-skill directly.
